...
This also writes two files to the `output\` folder:
- `output\cpe-config.csv` — all tunnel parameters in one place (IPs, IDs, PSK)
- `output\configure-tunnels.sh` — the Bash version of the configure script (for Mac/Linux use)
...
The PowerShell script `aruba\configure_tunnels.ps1` reads your Terraform output and pushes the IPsec tunnel configuration and VTIs to each EdgeConnect appliance via the ECOS REST API.
...
- `terraform apply` must have completed successfully (the script reads its output)
...
The remove script reads the same Terraform output to know which tunnels to delete. It will:
...
> **Important:** Run this **before** `terraform destroy`. The Windows remove script requires the Terraform state file to identify tunnel names. Once you run `terraform destroy`, the state is gone and the script will have nothing to read.

**Specific appliances only:**
```powershell
.\aruba\remove_tunnels.ps1 -Sites "10.0.0.1,10.0.0.2"
```
...
2. Run `terraform apply -parallelism=1` — only new resources will be created.
3. Run `.\aruba\configure_tunnels.ps1 -Sites "new-appliance-ip"` to configure only the new appliances.

### Removing Sites

1. Delete the site's row from `sites.csv`.
2. Run `.\aruba\remove_tunnels.ps1 -Sites "appliance-ip"` to clean up the appliance first.
...
The `configure_tunnels.ps1` script is idempotent on the appliance side (skips existing tunnels). If a tunnel is stuck in a partial state:
1. Run `remove_tunnels.ps1` to clean up the appliance side
2. Run `terraform apply -parallelism=1` to ensure the Cloudflare side is correct
3. Run `configure_tunnels.ps1` to push config to the appliances

### Sites with blank `customer_gw_ip` (NAT'd sites)
The PowerShell script does not support automatic WAN IP resolution from the Orchestrator. For NAT'd sites, the tunnel source is set to `0.0.0.0`, meaning the appliance will initiate IKE from its current WAN IP. If you need to specify a static source IP for a previously-dynamic site, add the WAN IP to `customer_gw_ip` in `sites.csv` and re-run `terraform apply` followed by `configure_tunnels.ps1`.
...
| `aruba\configure_tunnels.ps1` | PowerShell script to configure EdgeConnect appliances |
| `aruba\remove_tunnels.ps1` | PowerShell script to remove tunnels and VTIs from appliances |
...
All three are gitignored. Keep them on your local machine only. Do not email, share, or commit them.
