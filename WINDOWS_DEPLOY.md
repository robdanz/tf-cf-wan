...
- **`customer_gw_ip`** — informational only; it does not affect tunnel configuration (the appliance's WAN IP is always resolved dynamically from the Orchestrator when `configure-tunnels.ps1` runs). If an appliance has multiple WAN interfaces and you want a specific one recorded here for reference, edit as needed — it's fine to leave as detected or blank either way.
...
| `customer_gw_ip` | No | Informational only — does not affect tunnel configuration. The appliance's WAN IP is always resolved dynamically via Orchestrator when `configure-tunnels.ps1` runs, regardless of this value. Fine to leave blank. |
...
**`-Orchestrator` is required for every site** when running `configure-tunnels.ps1`, regardless of `customer_gw_ip` — the appliance's current WAN IP is always resolved dynamically at run time. See Part 6.
...
- Network access to the Orchestrator and `$env:ARUBA_API_TOKEN` set — required for every site, since the appliance's WAN IP is always resolved dynamically at run time
...
`-Orchestrator` is required for every site — the script always resolves each appliance's current WAN IP from the Orchestrator at run time, regardless of `customer_gw_ip`. Make sure `$env:ARUBA_API_TOKEN` is set (from Step 4A.2), then:
...
> These examples show one flag at a time for clarity — in practice, add `-Orchestrator ORCH_HOST` (and `$env:ARUBA_API_TOKEN`/`-OrchToken`) to all of them, since it's required on every run.
...
3. Run `.\output\configure-tunnels.ps1 -Orchestrator ORCH_HOST -Sites "new-appliance-ip"` to configure only the new appliances.
...
> **Testing both Windows and Mac/Linux separately, against separate Terraform state?** Each has its own gitignored `terraform.tfvars`. Make sure the health-check and IPsec values match across both — a `terraform.tfvars` copied from an old `terraform.tfvars.example` can carry stale defaults that silently diverge from the other deployment. Regenerate both from the current `terraform.tfvars.example` if in doubt.
...
All appliances are behind NAT or have dynamic WAN IPs (or the Orchestrator doesn't have current interface state). This is harmless — `customer_gw_ip` is informational only and doesn't affect tunnel configuration, since the appliance's WAN IP is always resolved dynamically at `configure-tunnels.ps1` run time regardless of this value.
...
Pass `-Orchestrator` (and `$env:ARUBA_API_TOKEN` or `-OrchToken`) to `output\configure-tunnels.ps1`. This is required for every site now, not just NAT'd ones — the Orchestrator is always needed to resolve the appliance's current WAN IP at run time, regardless of `customer_gw_ip`.

### Tunnel and VTI create successfully (shows "OK") but the tunnel never comes up
...
- `source` must be an address actually bound to the appliance's local WAN interface. `configure-tunnels.ps1` always resolves this dynamically now, preferring the interface's real `ipv4` over Orchestrator's `publicIp` field — a NAT-translated public IP isn't assigned to any local interface and can't be used to originate packets.
