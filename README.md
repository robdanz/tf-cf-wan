...
> Both `output/remove-tunnels.sh` and `output/remove-tunnels.ps1` are self-contained — they do not require Terraform or the tfstate file. You can run them before or after `terraform destroy`.
...
The PowerShell script `output\configure-tunnels.ps1` is generated by `terraform apply` with the PSK and all tunnel parameters embedded. Run it from the repo root to push IPsec configuration and create VTIs on each appliance. You will be prompted once for the EdgeConnect admin password.
...
**NAT'd or dynamic sites** (blank `customer_gw_ip`): The script uses `ec_hostname` as the IPsec tunnel source IP. NAT traversal (NAT-T) handles the public IP mapping automatically — no Orchestrator is needed.
...
```powershell
.\output\remove-tunnels.ps1 -DryRun    # preview
.\output\remove-tunnels.ps1            # execute
.\output\remove-tunnels.ps1 -Sites "10.0.0.1,10.0.0.2"  # specific appliances only
```

> `output\remove-tunnels.ps1` is self-contained — it does not need Terraform or the tfstate file. You can run it even after `terraform destroy`.
...
**NAT'd sites on Mac/Linux**
Pass `--orchestrator` to `output/configure-tunnels.sh` so it can resolve the live WAN IP at run time. On Windows, `output\configure-tunnels.ps1` uses `ec_hostname` as the source IP and relies on NAT-T — no Orchestrator needed.
