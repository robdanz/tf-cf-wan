...
> Run the remove script **before** `terraform destroy`. `output/remove-tunnels.sh` is self-contained (no terraform needed), but the Windows `aruba/remove_tunnels.ps1` reads terraform output and requires state to be present.
...
| Script | Platform | Description |
|---|---|---|
| `output/configure-tunnels.sh` | Mac/Linux | Generated by `terraform apply`; PSK embedded; run directly |
| `aruba/configure_tunnels.ps1` | Windows | Reads terraform output at run time; requires terraform in PATH |

### Remove tunnels

| Script | Platform | Description |
|---|---|---|
| `output/remove-tunnels.sh` | Mac/Linux | Generated by `terraform apply`; self-contained; works after `terraform destroy` |
| `aruba/remove_tunnels.ps1` | Windows | Reads terraform output at run time; requires terraform in PATH |

### Common flags

| Flag | Mac/Linux | Windows (PowerShell) | Description |
|---|---|---|---|
...
Sites with a blank `customer_gw_ip` need the Orchestrator to resolve the live WAN IP at configure time. Pass `--orchestrator HOST` to `output/configure-tunnels.sh` (Mac/Linux) or use `-Orchestrator HOST` with `aruba/configure_tunnels.ps1` (Windows).
