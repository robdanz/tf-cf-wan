...
**NAT'd or dynamic sites** (blank `customer_gw_ip`): The script needs to resolve the appliance's current WAN IP from the Orchestrator. Make sure `$env:ARUBA_API_TOKEN` is set, then pass `-Orchestrator`:
